 ...
 * @LastEditors: IMAGINE
 * @LastEditTime: 2021-06-03 00:33:46
-->
<template>
  <div class="tinypng-wrapper">
    <div class="tips">
      <p>1. 将jpg格式图片 <span class="highlight">png</span> 格式图片；</p>
      <p>2. 一次最多转换<span class="highlight">100张</span>；</p>
      <p>
        3. 转换后的文件会保存在<span class="highlight"
          >选择的路径下的transfromPNG文件夹</span
        >中, 请留意成功后的提示；
      </p>
      <p>
        4. transfromPNG文件夹中如果有同名文件，将被<span class="highlight"
          >覆盖</span
        >；
      </p>
...
export default {
  setup() {
    ...
    const handleSubmit = () => {
     ...
     ipcRenderer.send("transform-png", data);
    };
    ...
    const fileChange = () => {
      try {
        ...
        if (fu == null) return
        targetDir.value = PATH.parse(fu.files[0].path).dir+ `${PATH.sep}transformPNG`
    ...
    // 响应主进程推送的图片压缩状态，并弹框显示
          ipcRenderer.on("png-status", (event, arg) => {
        ElNotification({
